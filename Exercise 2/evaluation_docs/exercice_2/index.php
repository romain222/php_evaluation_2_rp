...
if(!empty($_POST)){
	...
	// wrong function | changed filter_variables() to filter_input()
    if(!filter_input(INPUT_GET, $post['email'], FILTER_VALIDATE_EMAIL)){
	...
	// bindValue -> removed ':' from first parameter
	if(count($errors) > 0){
...
// moved this code block outside of POST condition so the list is visible all the time
